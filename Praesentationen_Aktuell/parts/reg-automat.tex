...
\begin{frame}<handout:0>{Lösung}
    \begin{center}
        \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3.2cm,semithick]
            ...
            \path   (0) edge [bend left=10,pos=0.6] node {\footnotesize\framebox{EG$\nearrow$1.OG}} (1)
            edge [pos=0.7]      node {\footnotesize\framebox{OFF}} (E)
            edge [bend left=90] node {\footnotesize\framebox{EG$\nearrow$2.OG}} (2)
            (1) edge [bend left=10,pos=0.4] node {\footnotesize\framebox{1.OG$\nearrow$2.OG}}(2)
            edge [bend left=10,pos=0.4] node {\footnotesize\framebox{1.OG$\searrow$EG}} (0)
            (2) edge [bend left=10,pos=0.6] node {\footnotesize\framebox{2.OG$\searrow$1.OG}}(1)
            edge [bend left=90] node {\footnotesize\framebox{2.OG$\searrow$EG}} (0);
        \end{tikzpicture}
    \end{center}
\end{frame}
}
