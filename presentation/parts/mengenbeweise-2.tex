...
\begin{frame}{Weiterer Mengenbeweis}
    ...
    Sei $x$ beliebig.\\
    Angenommen, $x \in L_1$.\\
    Es gilt: $w \in \{aaaa\}$. Damit gilt $|w|=4$. Es folgt $|x| = |w^{n}| = |w| * n = 4 * n$ mit $n \in \mathbb{N}$. Daraus folgt $|x| \equiv 0 \pmod 4$. Weiterhin gilt $(aaaa)^n \in \{a\}^*$.\\
    $\leadsto x \in L_2$
    \end{block}
    }
    \only<2|handout:1>{
    ...
    Sei $x$ beliebig.\\
    Angenommen, $x \in L_2$.\\
    Es gilt: $|x| \equiv 0 \pmod 4$. \\ Damit gilt $|x| = 4 * n = |w| * n = |w^n|$ mit $w \in\{aaaa\}$ und $n \in \mathbb{N}$. \\ Weiterhin gilt $w \in \{a\}^*$. \\
    $\leadsto x \in L_1$
    \end{block}
    }
    \only<3|handout:2>{
