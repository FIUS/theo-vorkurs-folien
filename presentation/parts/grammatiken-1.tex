...
\begin{frame}[fragile]{Beispiel Worterzeugung}
    ...
    \item <3-> 
            ...
            \alert<4|handout:0>{\onslide<4->{
            \begin{align*}\alert<6,8|handout:0>{S \rightarrow aSa}&\text{ oder }\alert<7,9|handout:0>{S \rightarrow bSb}\\\text{oder }S \rightarrow aa &\text{ oder }\alert<10|handout:0>{S \rightarrow bb}\end{align*}}}\vspace{-0.3in}
    \item <5->
            ...
            $\leadsto$ 
            \only<5|handout:0>{\alert<5>{$S$}}\only<6|handout:0>{\alert<6>{$aSa$}}\only<7|handout:0>{a\alert<7>{$bSb$}$a$}\only<8|handout:0>{$ab$\alert<8>{$aSa$}$ba$}\only<9|handout:0>{$aba$\alert<9>{$bSb$}$aba$}\only<10-|handout:0>{$abab$\alert<10>{$bb$}$baba$}
...
\begin{frame}{Weitere Beispiele für Produktionen}
    ...
    $\onslide<5->{P=\{S \rightarrow  aS \only<6->{\ |\ bS}\only<7->{\ |\ \emptyWord\}}}$
    \end{exampleblock}
	}
    \only<8->{
	...
	$\begin{aligned}
		...
		\onslide<10->{A & \rightarrow aA \ |\ \emptyWord,} \\
		\onslide<11->{B & \rightarrow bb,} \\
		\onslide<12->{C & \rightarrow cC \ |\ \emptyWord \} }			
	\end{aligned}$
	\end{exampleblock}
	}
\end{frame}
