...
\begin{frame}[fragile]{Was ist Aussagenlogik?}
    ...
    \begin{itemize}
        ...
        \item Wie oft trifft Mei auf Totoro?
        \item $(x+y)^2+1$
        \item $\{a,b,c\}$
        \item ...
    \end{itemize}
...
\begin{itemize}
    \item $A$: Totoro möchte Nüsse.
    \item $B$: Totoro will schlafen.
\end{itemize}
\metroset{block=fill}
\begin{alertblock}{Grundoperationen}
\begin{itemize}
    \item<1-> \textbf{Und}: $A$ \alert<1>{$\wedge$} $B$ $\leadsto$ Totoro möchte Nüsse \alert<1>{und} will schlafen.\\
    \only<1>{\emph{Analog}: $M$: $M_1 \cap M_2$, Jedes Element aus $M$ liegt in $M_1$ \textbf{und} in $M_2$}
    \item<2-> \textbf{Oder}: $A$ \alert<2>{$\vee$} $B$ $\leadsto$ Totoro möchte Nüsse \alert<2>{oder} will schlafen.\\
    ...
    \item<3> \textbf{Nicht}: \alert<3>{$\neg$}$A$ $\leadsto$ Totoro möchte \alert<3>{keine} Nüsse.\\
...
{\setbeamercolor{palette primary}{bg=ExColor}
	\begin{frame}{Denkpause}
		...
		\begin{block}{Normal bis Schwer}
			\begin{itemize}
				\item Aussage: "Wenn Lukas nicht im Vorkurs,  schläft er noch"\\
				Geschehen: Lukas ist nicht im Vorkurs.\\
				Folgerung: Also schläft er noch.
				\item Aussage: "Wenn Mei nicht rennt, bekommt sie den Bus nicht"\\
				Geschehen: Mei bekommt den Bus.\\
...
{\setbeamercolor{palette primary}{bg=ExColor}
	\begin{frame}<handout:0>{Lösungen}
		...
		\only<2>{
			...
			\begin{block}{Normal bis Schwer}
				\begin{itemize}
					\item<2> Aussage: "Wenn Mei nicht rennt, bekommt sie den Bus nicht"\\
					Geschehen: Mei bekommt den Bus.\\
					Folgerung: Also ist sie gerannt.
				\end{itemize}
			\end{block}
...
\begin{frame}[fragile]{Denkpause}
    ...
    \begin{itemize}
        ...
        \item $A_8$: Wenn es Totoro in echt gibt,\\
		\quad dann frisst Totoro kleine Kinder.
    \end{itemize}
    \end{block}
\end{frame}
}
