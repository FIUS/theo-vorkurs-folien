...
\begin{frame}[fragile]{Logische Operationen}
	...
	\begin{itemize}
		\item $A$: Hamster Paule möchte Nüsse.
		\item $B$: Hamster Paule will schlafen.
	\end{itemize}
	\metroset{block=fill}
	\begin{alertblock}{Grundoperationen}
		\begin{itemize}
			\item<1-> \textbf{Und}: $A$ \alert<1>{$\wedge$} $B$ $\leadsto$ Hamster Paule möchte Nüsse \alert<1>{und} will schlafen.\\
			      \only<1>{\emph{Analog}: $M$: $M_1 \cap M_2$, Jedes Element aus $M$ liegt in $M_1$ \textbf{und} in $M_2$}
			\item<2-> \textbf{Oder}: $A$ \alert<2>{$\vee$} $B$ $\leadsto$ Hamster Paule möchte Nüsse \alert<2>{oder} will schlafen.\\
			...
			\item<3> \textbf{Nicht}: \alert<3>{$\neg$}$A$ $\leadsto$ Hamster Paule möchte \alert<3>{keine} Nüsse.\\
...
\begin{frame}{Wahrheitstabellen}
	Der Wahrheitswert einer Aussage kann in Abhängigkeit der Teilaussagen in einer Tabelle dargestellt werden:
	\par\begin{center}
		\begin{tabular}{c | c | c | c | c}
			$A$        & $B$ & $A\land B$ & $A\lor B$ & $\lnot A$ \\ \hline
			0 (falsch) & 0   & 0          & 0         & 1         \\
			0          & 1   & 0          & 1         & 1         \\
			1 (wahr)   & 0   & 0          & 1         & 0         \\
			1          & 1   & 1          & 1         & 0         \\
...
\begin{frame}{Überblick: Mengenoperationen und logische Operatoren}

	{
		...
		\begin{multicols}{2}
			...
			Auf \alert{Mengen} $M_1$, $M_2$ lassen sich verschiedene Mengenoperationen ausführen.
			\metroset{block=fill}
			\begin{exampleblock}{Mengenoperationen}
				\begin{itemize}
					\item<2-|alert@2> Schnitt: $M_1 \cap M_2$
					\item<3-|alert@3> Vereinigung: $M_1 \cup M_2$
					\item<4-|alert@4> Komplement: $\overline{M_1}$
				\end{itemize}
			\end{exampleblock}
			\vfill
	...
	\only<5->{
		\alert{ACHTUNG:} Eine Mengenoperation kann \textbf{nicht} auf Aussagen angewandt werden und logische Operatoren \textbf{nicht} auf Mengen.
	}
\end{frame}
...
\begin{frame}{Logische Operationen: Implikation}
	\note{Ggf. hilfreich Wahrheitstabelle für Kontraposition an die Tafel zu malen}
...
\begin{frame}<handout:0>{Lösungen}
	...
	\only<2>{
		...
		\begin{block}{Normal bis Schwer}
			\begin{itemize}
				\item<2> Aussage: "Wenn Anne nicht rennt, bekommt sie den Bus nicht"\\
				      Geschehen: Anne bekommt den Bus.\\
				      Folgerung: Also ist sie gerannt.
			\end{itemize}
		\end{block}
	...
	\only<1>{Die Folgerung ist richtig. Laut Aussage ist Philipp im Vorkurs oder schläft noch (oder beides). Wenn er also nicht im Vorkurs ist, bleibt nur noch die Option übrig, dass er noch schläft.}
