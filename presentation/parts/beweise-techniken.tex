...
\subsection{Beweistechnik: Kontraposition}
...
\subsection{Beweistechnik: Widerspruch}
\begin{frame}[fragile]{Beweis durch Widerspruch}
	\small{
		...
		\begin{exampleblock}{Beispiel}
			...
			\note{Es muss $q \in \mathbb{Z} \setminus \{0\}$ sein, weil wir in der Umformung mit $q^2$ multiplizieren; sonst würde mit $0$ multipliziert}
			\begin{enumerate}
				\item<1-|handout:1> \alert<1|handout:0>{Ang. $\sqrt{2}$ ist rational.}
				\item<2-|handout:1> \alert<2|handout:0>{Dann $\exists p \in \mathbb{Z} \ q \in \mathbb{Z} \setminus \{0\} : \sqrt{2} = \frac{p}{q}$} $\wedge$ \alert<3,11|handout:0>{$p, q$ sind teilerfremd}
				...
				\item<4-|handout:1> \only<4|handout:0>{Quadrieren und Umformen:\\}$\leadsto(\sqrt{2})^2 = (\frac{p}{q})^2 \implies 2 = \frac{p^2}{q^2} \iff \alert<5,8|handout:0>{2q^2=p^2}$
