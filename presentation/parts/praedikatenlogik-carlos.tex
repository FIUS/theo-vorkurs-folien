...
\begin{frame}<handout:0>{Lösungen}
	...
	\only<3>{
		\begin{tikzpicture}[
			...
			\node[base] (n0) at (0,0){$x$};
			\node[base, right = of n0] (n1) {$z_0$};
			\node[right = of n1] (n2) {$\dots$};
			\node[base, right = of n2] (n3) {$z_n$};
			\node[base, right = of n3] (n4) {$y$};

			\draw[thick,->] (n0) to (n1);
			\draw[thick,->] (n1) to (n2);
			\draw[thick,->] (n2) to (n3);
			\draw[thick,->] (n3) to (n4);
			\draw[thick,->,bend left] (n4) to (n0);
		\end{tikzpicture}
	}
	\par
	\only<1>{
	...
	\only<3>{
		Allgemein:\\
		Für ein $n\in\mathbb{N}$ mit $n\geq 2$\\
		$U=\{0,\dots,n-1\}$\\
		$I_3(P_5) = \{(x,x+1)\mid x\in \mathbb{N} \land x < n-1\} \cup \{(n-1,0)\}$
	}
\end{frame}
	...
	\begin{block}{}
		\begin{multicols}{2}
			\begin{itemize}
				...
				\item $F_2 = \forall x: \lnot P_0(x,x)$
				\item $F_3 = \exists y \forall x: \lnot P_0(x,y)$
				\item $F_4 = \forall x: \lnot P_0(x,f(x))$
				\item {\scriptsize$F_5 = \forall x \forall y: (P_0(x,f(y)) \implies P_0(y,x))$}
				\item $F_6 = P(a, f(f(a)))$
				\item $F_7 = \forall x (\exists y: P_0(Y,X)\implies P_1(X))$
				\item $F_8 = \lnot P_1(f(f(b)))$
				\item $F_9 = P(f(c),c)$
			\end{itemize}
		\end{multicols}
