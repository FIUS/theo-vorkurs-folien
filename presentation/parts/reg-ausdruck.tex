...
\begin{frame}{Reguläre Ausdrücke verwenden}
    \begin{alertblock}{Induktive Definition der Syntax}
        \begin{itemize}
            ...
            \item Wenn \alert{$\alpha$} und \alert{$\beta$} reguläre Ausdrücke sind,\\dann sind \alert{$(\alpha)^*$}, \alert{($\alpha \ |\ \beta$)} und \alert{$\alpha \beta$} auch reguläre Ausdrücke.
        \end{itemize}
    \end{alertblock}
    \metroset{block=fill}
...
\begin{frame}{Bedeutung von regulären Ausdrücken}
    ...
    \begin{block}{Bedeutung von regulären Ausdrücken}
        ...
        \begin{itemize}[<+- | alert@+ | handout:+- | handout:alert@+>]
            ...
            \item Sei $\gamma = (\alpha\ |\ \beta)$: Dann ist $L(\gamma) = L(\alpha) \cup L(\beta)$
            \item Sei $\gamma = \alpha \beta$: Dann ist $L(\gamma) = \{uv \mid u \in L(\alpha) \wedge v \in L(\beta)\}$
        \end{itemize}
    \end{block}
    \metroset{block=fill}
    \begin{exampleblock}{
            \only<1|handout:1>{Beispiel: $\gamma = (\alpha)^*$}
            \only<2|handout:2>{Beispiel: $\gamma = (\alpha \mid \beta)$}
        ...
        \only<3|handout:3>{
            \begin{itemize}
                ...
                \item $L((bab | (ab)^*) abb) = \{bababb\} \cup \{(ab)^n abb \mid n \in \mathbb{N}\}$
            \end{itemize}
        }
    \end{exampleblock}
\end{frame}
