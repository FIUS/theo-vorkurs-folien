...
\begin{frame}{Wortlängen}
    ...
    \begin{itemize}
        ...
        \item Wort der Länge \alert<3|handout:0>{0}: \alert<3|handout:0>{$\emptyWord$}
    \end{itemize}
    \only<1-2>{Wir schreiben \alert<1>{$\absval{w}$} um \alert<1>{Länge des Wortes $w$} abzukürzen.\\}
    \only<2>{Um \alert<2|handout:0>{nur ein Symbol} (z.B. $a$) zu zählen verwenden wir \alert<2|handout:0>{$|w|_a$}.\\}
