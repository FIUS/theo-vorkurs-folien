...
\begin{frame}<handout:0>{Lösung: Schwer}
	...
	\begin{itemize}
		\item<only@1> wir in Zustand $q_i$ sind gdw. $i \equiv |w|_a \pmod{3}$
		\item<only@2> wir in Zustand $p_1$ sind gdw. zuletzt ein $a$ eingelesen wurde, sonst $p_0$.
	\item<only@3> wir in $(q_i, p_j)$ akzeptieren falls $i = 1$ (da $w \in L_1$) oder $j = 1$ (da $w \in L_2$)
	\end{itemize}
        \begin{center}
	    \scalebox{0.85}{
            \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
		...
		\path<1> (00)    edge[orange]                node[right] {$a$}    (11)
		...
		\node<3> [state, accepting,orange]    (12) [right of=11]    {${\color{black}q_2,} p_1$};
                ...
                \path<3> (00)    edge                node[right] {$a$}    (11)
                edge[loop above]    node        {$b$}    (00)
