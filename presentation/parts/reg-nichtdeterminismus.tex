...
\begin{frame}<handout:0>{Lösung: Schwer}
    \begin{alertblock}{DEA für $L_3 = L_1 \cup L_2$}
    	Idee: Wir nutzen die Zustände $\{(q_i, p_j) \mid 1 \leq i \leq 3, 1 \leq j \leq 2\}$, so dass
    	\begin{itemize}
            \only<1>{\item<only@1> wir in Zustand $q_i$ sind gdw. $i \equiv |w|_a \pmod{3}$}
    		\only<2>{\item<only@2> wir in Zustand $p_1$ sind gdw. zuletzt ein $a$ eingelesen wurde, sonst $p_0$.}
    		\only<3->{\item<only@3> wir in $(q_i, p_j)$ akzeptieren falls $i = 1$ (da $w \in L_1$) oder $j = 1$ (da $w \in L_2$)}
    	\end{itemize}
        \begin{center}
            \scalebox{0.85}{
            \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2cm,
                    semithick]
        	\node<1> [initial,state]       (00)                  {${\color{orange}q_0}, p_0$};
                \node<1> [state, accepting]    (01) [right of=00]    {${\color{orange}q_1}, p_0$};
                \node<1> [state]               (02) [right of=01]    {${\color{orange}q_2}, p_0$};
        	\node<1> [state, accepting]    (10) [below = 2cm of 00]    {${\color{orange}q_0}, p_1$};
        	\node<1> [state, accepting]    (11) [right of=10]    {${\color{orange}q_1}, p_1$};
        	\node<1> [state, accepting]    (12) [right of=11]    {${\color{orange}q_2}, p_1$};
        
        	\path<1> (00)    edge[orange]                node[right,pos=0.4] {$a$}    (11)
                edge[loop above]    node        {$b$}    (00)
        	(01)    edge[orange]                node[right] {$a$}    (12)
                edge[loop above]    node        {$b$}    (01)
        	(02)    edge[orange]                node[below,pos=0.38] {$a$}    (10)
                edge[loop above]    node        {$b$}    (02)
        	(10)    edge[orange]                node        {$a$}    (11)
                edge        node        {$b$}    (00)
        	(11)    edge[orange]                node        {$a$}    (12)
        	edge        node[pos=0.72]        {$b$}    (01)
                (12)    edge[bend left, orange]     node[below] {$a$}    (10)
                edge        node        {$b$}    (02);
        
        	\node<2> [initial,state]       (00)                  {$q_0, {\color{orange}p_0}$};
        	\node<2> [state, accepting]    (01) [right of=00]    {$q_1, {\color{orange}p_0}$};
        	\node<2> [state]               (02) [right of=01]    {$q_2, {\color{orange}p_0}$};
        	\node<2> [state, accepting]    (10) [below = 2cm of 00]    {$q_0, {\color{orange}p_1}$};
        	\node<2> [state, accepting]    (11) [right of=10]    {$q_1, {\color{orange}p_1}$};
        	\node<2> [state, accepting]    (12) [right of=11]    {$q_2, {\color{orange}p_1}$};
        
        	\path<2> (00)    edge[orange]               node[right,pos=0.4] {$a$}    (11)
                edge[loop above]    node        {$b$}    (00)
        	(01)    edge[orange]                node[right] {$a$}    (12)
                edge[loop above]    node        {$b$}    (01)
        	(02)    edge[orange]                node[below,pos=0.38] {$a$}    (10)
        	...
        	edge[orange]        node        {$b$}    (00)
                (11)    edge                node        {$a$}    (12)
        	edge[orange]        node[pos=0.72]        {$b$}    (01)
                (12)    edge[bend left]     node[below] {$a$}    (10)
        	edge[orange]        node        {$b$}    (02);
        
        	\node<3> [initial,state]       (00)                  {$q_0, p_0$};
        	\node<3> [state, accepting,orange]    (01) [right of=00]    {$q_1{\color{black},p_0}$};
                \node<3> [state]               (02) [right of=01]    {$q_2, p_0$};
        	\node<3> [state, accepting,orange]    (10) [below = 2cm of 00]    {${\color{black}q_0,} p_1$};
                \node<3> [state, accepting,orange]    (11) [right of=10]    {$q_1, p_1$};
        	\node<3> [state, accepting,orange]    (12) [right of=11]    {${\color{black}q_2,} p_1$};
        
                \path<3> (00)    edge                node[right,pos=0.4] {$a$}    (11)
                edge[loop above]    node        {$b$}    (00)
                (01)    edge                node[right] {$a$}    (12)
                edge[loop above]    node        {$b$}    (01)
                (02)    edge                node[below,pos=0.38] {$a$}    (10)
                edge[loop above]    node        {$b$}    (02)
                (10)    edge                node        {$a$}    (11)
                edge        node        {$b$}    (00)
                (11)    edge                node        {$a$}    (12)
