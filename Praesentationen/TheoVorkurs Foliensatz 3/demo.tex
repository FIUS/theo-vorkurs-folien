...
% Not necessary when using the XeLaTeX compiler
...
\begin{frame}{Mengenbeweis}
    ...
    $L_2=\{w \mid |w| \equiv 0 \mod 4, w \in \{a\}^*\}$
    \end{block}
    Zu zeigen:\\
    $L_1 = L_2$\\
...
\begin{frame}{Mengenbeweis}
    ...
    Sei $x$ beliebig.\\
    Angenommen, $x \in L_1$.\\
    Es gilt: $w \in \{aaaa\}$. Damit gilt $|w|=4$. Es folgt $|x| = |w^{n}| = |w| * n = 4 * n$ mit $n \in \mathbb{N}$. Daraus folgt $|x| \equiv 0 \mod 4$. Weiterhin gilt $(aaaa)^n \in \{a\}^*$.\\
    $\leadsto x \in L_2$
    \end{block}
    }
    \only<2>{
    ...
    Sei $x$ beliebig.\\
    Angenommen, $x \in L_2$.\\
    Es gilt: $|x| \equiv 0 \mod 4$. \\ Damit gilt $|x| = 4 * n = |w| * n = |w^n|$ mit $w \in\{aaaa\}$ und $n \in \mathbb{N}$. \\ Weiterhin gilt $w \in \{a\}^*$. \\
    $\leadsto x \in L_1$
    \end{block}
    }
    \only<3>{
...
\begin{frame}[fragile]{Denkpause}
    ...
    \metroset{block=fill}
    \begin{block}{Schwer}
    $L_1$: $\{w\mid |w|\equiv 0 \mod 6\}$\\
    $L_2$: $\{w\mid |w|\equiv 0 \mod 2\}$\\
    $L_3$: $\{w\mid |w|\equiv 0 \mod 3\}$\\
    \vspace{0.3cm}
...
\begin{frame}[fragile]{Lösung}
    ...
    \begin{alertblock}{"$\implies$"}
        Sei x beliebig. Ang. $x \in L_1$.\\
        Dann gilt $|x|\equiv 0 \mod 6$. Also $\exists k \in \mathbb{N}: |x| = 6k = 3 * 2 * k$.\\
